...
Subject: EM algorithm using Cholesky decomposition

Description:
We have replaced covariance estimation with
cholesky estimation. Since our Covariance estimation
returns a positive definite covariance matrix, we
believe the Cholesky decomposition approach should
return the same parameter estimates. 

We will be comparing the two methods. If they return
the same results we will use the Cholesky method going
forward.
"""
...
# Remove once all is good and well and working and in good health and properly medicated
...
def cholFct(pS, y, mu, S, A):   
    covm = [[covFct(pS[s, :], y[i,:], y[j,:], mu[i,s], mu[j,s]) for i in range(A) for j in range(A)] for s in range(S)]
    covm = np.array(covm).reshape(S,A,A)
...
def EM(returns, sims, mat, states, assets, p, pS):
    ...
    chol = cholFct(ps, returns, mu, states, assets)
