...
g     = 5
seed  = 23456
muS  = np.array([-11.060,0.608,1.708])
muB  = np.array([  1.975,0.115,0.833])
mu    = np.array((muS,muB))
cov =  np.array([[[ 8.181,-0.456],
                  [-0.456, 3.936]],
                 [[ 3.748, 0.375],
                  [ 0.375, 1.995]],
                 [[ 4.891, 0.553],
...
for i in range(len(maturities)):
    ...
    weights[i] = results.x
plt.plot(maturities, weights[:,0], label = 'Stocks')
plt.plot(maturities, weights[:,1], label = 'Bonds')
plt.plot(maturities, weights[:,2], label = 'T-bills')
plt.legend()
plt.show()
